...
def get_sentence_level_bigram_counter (parsed_reviews):
    ...
    bigrams = [set() for doc in reviews_as_sents]
    for doc_id, rev_doc in enumerate(reviews_as_sents):
        for sent in rev_doc:
            for i, w_i in enumerate(sent):
                for j, w_j in enumerate(sent[i + 1:]):
                    bigrams[doc_id].add((w_i, w_j))
                    bigrams[doc_id].add((w_j, w_i))
    bigrams = [bigram for bigram_set in bigrams for bigram in bigram_set]
    bigram_doc_counter = Counter(bigrams)
    return bigram_doc_counter


def get_statistical_sim_mat (words, parsed_reviews, X_tag_freq):
    f_xi = {w:X_tag_freq[w]['freq'] for w in words}
